...
function helpSection(event) {
  ...
  alert(
    "I am supposed to show you something about " +
      `${id[0].toUpperCase() + id.slice(1)} and stuff like that.`
  );

  if (id === "terms") {
    alert(
      "But this calculator is a template modified by me so I didn't add any terms of use etc."
    );
  } else if (id === "privacy") {
    privacyTexts = [
      "This is a website about a calculator, there's " +
        "no need to set any kind of privacy policy.",

      "But I'm trying to redirect you from this page to another" +
        " one, so, let's look at the formal definition of privacy.",
    ];

    for (text of privacyTexts) {
      alert(text);
    }
